...
local pomodoro_duration = 25 * 60 * 1000000000 -- 25 minutes in nanoseconds
local short_break_duration = 5 * 60 * 1000000000 -- 5 minutes in nanoseconds
local long_break_duration = 15 * 60 * 1000000000 -- 15 minutes in nanoseconds
local pomodoro_sessions_completed = 0
...
function play_chime()
    local source = obs.obs_get_source_by_name("Pomodoro_Chime")
    if source ~= nil then
...
function set_time_text(ns, text)
    ...
    -- Pomodoro session info added to text
    local pomodoro_status_text = is_work_session and "werk" or "stretch"
    local session_info = string.format(" [%d/4] %s", pomodoro_sessions_completed % 4, pomodoro_status_text)
    text = text .. session_info
...
function script_tick(sec)
    ...
    if cur_ns < 1 then
        if is_work_session then
			play_chime()
            pomodoro_sessions_completed = pomodoro_sessions_completed + 1
            is_work_session = false
            cur_time = (pomodoro_sessions_completed % 4 == 0) and long_break_duration or short_break_duration
        else
...
function script_description()
	return "Sets a text source to act as a timer with advanced options. Hotkeys can be set for starting/stopping and resetting the timer. Includes Pomodoro functionality."
end
...
function script_update(settings)
    ...
    -- Set timer to Pomodoro duration if Pomodoro mode is active
